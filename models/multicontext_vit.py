...
class ViT(nn.Module):
    def __init__(self, *, image_size, image_patch_sizes, frames, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.,conv_model= None,enable_maxpool= False,conv_dimension= 64):
        super().__init__()
    ...
    def forward(self, video):
        ...
        for i,rearrange in enumerate(self.rearrange_patches):
            ...
            if embeddings is None:
                ...
                pos_embedding = self.pos_embeddings[i][:, :(self.total_num_volume_patches_per_context[i])]
                ...
                #repeating larger block embeddings that need to be concatanated with the smallest block embeddings
                context_embedding = torch.repeat_interleave(context_embedding, self.total_num_volume_patches_per_context[0]//self.total_num_volume_patches_per_context[i] ,dim = 1)
                
                
                #concatanating multi context embeddings
                embeddings = torch.cat((embeddings,context_embedding), dim=2)
