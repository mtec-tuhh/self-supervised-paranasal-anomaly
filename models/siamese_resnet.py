...
class SpatialAttentionTiny(nn.Module):
    def __init__(self, channel_size = 3, kernel_size=7):
        super(SpatialAttentionTiny, self).__init__()
        

        self.conv1 = nn.Conv3d(2, 1, kernel_size, padding=kernel_size//2, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x_org,x_residual):
        
        x_residual = torch.nn.functional.interpolate(x_residual,size=(x_org.shape[2],x_org.shape[3],x_org.shape[4]))
        avg_out = torch.mean(x_residual, dim=1, keepdim=True)
        max_out, _ = torch.max(x_residual, dim=1, keepdim=True)
        x = torch.cat([avg_out, max_out], dim=1)
...
class SiameseResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_attn_1 = SpatialAttentionTiny(kernel_size=7)
        self.spatial_attn_2 = SpatialAttentionTiny(kernel_size=7)
        self.spatial_attn_3 = SpatialAttentionTiny(kernel_size=7)
        self.spatial_attn_4 = SpatialAttentionTiny(kernel_size=3)
        self.spatial_attn_5 = SpatialAttentionTiny(kernel_size=3)
    ...
    def forward(self, x,x_res):
        ...
        if self.spatial_attn[0] == 1:
            
            x = self.spatial_attn_1(x,x_res) 
        ...
        if self.spatial_attn[1] == 1:
            
            x3 = self.spatial_attn_2(x3,x_res) 
        ...
        if self.spatial_attn[2] == 1:
            
            x4 = self.spatial_attn_3(x4,x_res) 

        

        x5 = self.layer3(x4)

        x5 = self.spatial_attn_4(x5,x_res) 

        if self.spatial_attn[3] == 1:
        ...
        x6 = self.layer4(x5)
        x6 = self.spatial_attn_4(x6,x_res) 

        if self.spatial_attn[4] == 1:
            
            x6 = self.spatial_attn_5(x6,x_res) 
...
def generate_siamese_resnet_model(model_depth, **kwargs):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = SiameseResNet(BasicBlock, [1, 1, 1, 1], get_inplanes(), **kwargs)
    elif model_depth == 18:
        model = SiameseResNet(BasicBlock, [2, 2, 2, 2], get_inplanes(), **kwargs)
    elif model_depth == 34:
        model = SiameseResNet(BasicBlock, [3, 4, 6, 3], get_inplanes(), **kwargs)
    elif model_depth == 50:
        model = SiameseResNet(Bottleneck, [3, 4, 6, 3], get_inplanes(), **kwargs)
    elif model_depth == 101:
        model = SiameseResNet(Bottleneck, [3, 4, 23, 3], get_inplanes(), **kwargs)
    elif model_depth == 152:
        model = SiameseResNet(Bottleneck, [3, 8, 36, 3], get_inplanes(), **kwargs)
    elif model_depth == 200:
        model = SiameseResNet(Bottleneck, [3, 24, 36, 3], get_inplanes(), **kwargs)
...
class ResNetClassifer(pl.LightningModule):
    ...
    def __init__(self,cfg,prefix=None):
        ...
        if self.cfg.enable_siamese_network: 
            self.model = generate_siamese_resnet_model(model_depth=self.cfg.model_depth,spatial_attn=self.cfg.spatial_attn)
        else:
            self.model = generate_resnet_model(model_depth=self.cfg.model_depth)
        #print(self.model)
    ...
    def configure_optimizers(self):
        ...
        self.optimizer =  optim.Adam(self.model.parameters() ,lr=self.cfg.lr, amsgrad=False)
        self.scheduler =  optim.lr_scheduler.ReduceLROnPlateau(optimizer=self.optimizer,verbose=True, patience=2)
        return {"optimizer": self.optimizer, "lr_scheduler": self.scheduler,"monitor": "val/loss"} #[optimizer],[scheduler]

    def prepare_batch(self, batch):
        
        return {"image": batch['one_image'][tio.DATA],
                "image_residuals": batch['one_image_residual'][tio.DATA],
                "label": batch['label'],
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputs_residual = return_object['image_residuals']

        # print(inputs.shape)
        target = return_object['label']
        
        outputs  = self.model(inputs,inputs_residual)
