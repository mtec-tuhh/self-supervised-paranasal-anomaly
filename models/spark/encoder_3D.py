...
def _get_active_ex_or_ii(H, returning_active_ex=True):
    downsample_raito = H[2] // _cur_active.shape[-1]
    active_ex = _cur_active.repeat_interleave(downsample_raito, 2).repeat_interleave(downsample_raito, 3).repeat_interleave(downsample_raito, 4)
    active_ex = active_ex[...,:H[-1]] # cut depth
    return active_ex if returning_active_ex else active_ex.squeeze(1).nonzero(as_tuple=True)  # ii: bi, hi, wi


def sp_conv_forward(self, x: torch.Tensor):
    x = super(type(self), self).forward(x)
    x *= _get_active_ex_or_ii(H=x.shape, returning_active_ex=True)    # (BCHW) *= (B1HW), mask the output of conv
    return x


def sp_bn_forward(self, x: torch.Tensor):
    ii = _get_active_ex_or_ii(H=x.shape, returning_active_ex=False)
...
class SparseMaxPooling_3D(nn.MaxPool3d):
    forward = sp_conv_forward   # hack: override the forward function; see `sp_conv_forward` above for more details


class SparseAvgPooling_3d(nn.AvgPool3d):
    forward = sp_conv_forward   # hack: override the forward function; see `sp_conv_forward` above for more details
...
class SparseEncoder(nn.Module):
    ...
    def dense_model_to_sparse(m: nn.Module, verbose=False, sbn=False):
        ...
        if isinstance(m, nn.Conv3d):
            ...
            m: nn.MaxPool3d
            oup = SparseMaxPooling_3D(m.kernel_size, stride=m.stride, padding=m.padding, dilation=m.dilation, return_indices=m.return_indices, ceil_mode=m.ceil_mode)
        elif isinstance(m, nn.AvgPool3d):
            m: nn.AvgPool3d
            oup = SparseAvgPooling_3d(m.kernel_size, m.stride, m.padding, ceil_mode=m.ceil_mode, count_include_pad=m.count_include_pad, divisor_override=m.divisor_override)
        elif isinstance(m, (nn.BatchNorm3d, nn.SyncBatchNorm)):
            m: nn.BatchNorm3d
            oup = (SparseSyncBatchNorm3d if sbn else SparseBatchNorm3d)(m.weight.shape[0], eps=m.eps, momentum=m.momentum, affine=m.affine, track_running_stats=m.track_running_stats)
            oup.weight.data.copy_(m.weight.data)
            oup.bias.data.copy_(m.bias.data)
