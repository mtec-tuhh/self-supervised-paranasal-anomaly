...
class SparK_2D(nn.Module):
    ...
    def forward(self, raw_inp: torch.Tensor, active=None):
        ...
        rec_bchw, recs = self.dense_decoder(to_dec)
        
        # calc loss
        mean, var, spatial_loss = self.spatial_loss(raw_inp, rec_bchw, active)
        return active_ex, rec_bchw, spatial_loss, fea_bcffs, recs
...
class SparK_2D_encoder(nn.Module):
    def __init__(self, cfg,chans=1,features_only=False):
        super().__init__()
        sbn=False
        ...
        self.encoder = build_encoder(backbone, 
                                    ...
                                    sbn=sbn, 
                                    drop_path_rate=cfg.get('dp',0), 
                                    verbose=False,chans=chans,features_only=features_only)
