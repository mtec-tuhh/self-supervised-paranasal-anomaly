...
import torchvision
from models.spark.convnext import ConvNeXt
from models.spark.resnet import ResNet
_import_resnets_for_timm_registration = (ResNet,)

from models.spark.resnet_3D import resnet50, resnet18, resnet34, resnet101, resnet152, resnet200
...
def build_sparse_encoder(name: str, input_size: int, sbn=False, drop_path_rate=0.0, verbose=False):
    from models.spark.encoder import SparseEncoder
...
def build_encoder(name: str, cond_dim:int, input_size: int, sbn=False, drop_path_rate=0.0, verbose=False,chans=1,features_only=False):
    ...
    print(f'[sparse_cnn] model kwargs={kwargs}')
    cnn = create_model(name,in_chans=chans,features_only=features_only, **kwargs)
...
def build_sparse_encoder_3D(name: str, input_size: int, sbn=False, drop_path_rate=0.0, verbose=False):
    from models.spark.encoder_3D import SparseEncoder

    downsample_raito = 32 # hack
    fea_dim = 512 # hack
    cnn = resnet18(sample_size=input_size, sample_duration=input_size, last_fc=False)


    
    return SparseEncoder(cnn, input_size=input_size, downsample_raito=downsample_raito, encoder_fea_dim=fea_dim, verbose=verbose, sbn=sbn)

def build_encoder_3D(name: str, cond_dim:int, input_size: int, sbn=False, drop_path_rate=0.0, verbose=False, chans=1):
    
    downsample_raito = 32 # hack
    fea_dim = 512 # hack
    cnn = resnet18(sample_size=input_size, sample_duration=input_size, last_fc=False, num_classes=cond_dim, chans = chans)
