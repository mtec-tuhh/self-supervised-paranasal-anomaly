...
class UNetBlock2x(nn.Module):
    def __init__(self, cin, cout, cmid, last_act=True):
        ...
        self.b = nn.Sequential(
            nn.Conv3d(cin, c_mid, 3, 1, 1, bias=False), _BN(c_mid), nn.ReLU6(inplace=True),
            nn.Conv3d(c_mid, cout, 3, 1, 1, bias=False), _BN(cout), (nn.ReLU6(inplace=True) if last_act else nn.Identity()),
        )
...
class DecoderConv(nn.Module):
    def __init__(self, cin, cout, double, heavy, cmid):
        super().__init__()
        self.up = nn.ConvTranspose3d(cin, cin, kernel_size=4 if double else 2, stride=2, padding=1 if double else 0, bias=True)
    ...
    def forward(self, to_dec):
        ...
        for i, d in enumerate(self.dec):
            if i < len(to_dec) and to_dec[i] is not None:
                if i != 0 and x.shape != to_dec[i].shape:
                    x = nn.functional.interpolate(x, size=to_dec[i].shape[2:], mode='trilinear', align_corners=False) # interpolate the size to match the next fmap
                x = x + to_dec[i]
            x = self.dec[i](x) 
            
        x = nn.functional.interpolate(x, size=(64, 64, 64), mode='trilinear', align_corners=False) # hack # TODO
        return self.proj(x)
