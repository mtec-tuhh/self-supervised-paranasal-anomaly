...
split_directory = "/home/debayan/Desktop/MRI_HCHS/JAMA-labelled-1000-corrected/splits/"
...
for size_of_crop in size_of_crops:
    ...
    for std in std_factors:
        ...
        root_save_path =  f"/home/debayan/Desktop/MRI_HCHS/JAMA-labelled-1000-corrected/crop_size_{size_of_crop}_std_factor_{std}/"
        ...
        for i, (train_index, val_index) in enumerate(skf.split(X_trainval, y_trainval)):
            ...
            y_train_autoencoders = np.array(y_train)

            X_train_autoencoders = list(np.array(X_train)[y_train_autoencoders==0])
            y_train_autoencoders = [0]*len(X_train_autoencoders)


            count_repetitions(y_train_autoencoders,"Training Set Distribution Autoencoders")
            count_repetitions(y_train,"Training Set Distribution")
            count_repetitions(y_val,"Validation Set Distribution")
        
            train_dataframe_autoencoders = pd.DataFrame.from_dict({"folder":X_train_autoencoders, "label": y_train_autoencoders})
            ...
            train_dataframe_autoencoders.to_csv(split_directory + f"/crop_size_{size_of_crop}_std_factor_{std}/fold_{fold}/" + "train_autoencoders.csv")
