...
def __get__crop__(data,location,total_samples=20,std_factor=1,size_of_crop=40,flip=False):
    ...
    # sample x and y from a line with regular intervals and keep z constant 
    arr_x = np.linspace(x - std_factor*x_std, x + std_factor*x_std, total_samples)
    #arr_x = [x]*total_samples
    #arr_y = np.linspace(y - std_factor*y_std, y + std_factor*y_std, total_samples)
    arr_y = [y]*total_samples
    #arr_z = np.linspace(z - std_factor*z_std, z + std_factor*z_std, total_samples)
    arr_z = [z]*total_samples
    ...
    for x,y,z in zip(arr_x,arr_y,arr_z):
        ...
        if flip:
            out =  np.array(np.flip(out, axis=0), dtype=np.float64)
...
for size_of_crop in [35]: #[15,20,25,30,35,40,45,50,55]
    ...
    for std_factor in [1]: #[1,2,3]:
        ...
        root_save_path =  f"/home/debayan/Desktop/MRI_HCHS/CARS-labelled-yz-constant/crop_size_{size_of_crop}_std_factor_{std_factor}/"
        root_split_path =  f"/home/debayan/Desktop/MRI_HCHS/CARS-labelled-yz-constant/splits/"

        create_dir(root_save_path)
        create_dir(root_split_path)


        df_label = pd.read_excel('//media/debayan/c7b64c90-ca4e-4192-8ed9-8fea1d005196/MRI_HCHS/KI HCHS Project ASH 14072022_CARS.xlsx',engine='openpyxl', index_col=False)
        ...
        for df in [df_unlabelled]:
            ...
            for index, row in tqdm(df.iterrows(),total=len(df)):
                ...
                roi_objects = __get__crop__(img.get_fdata(),cfg["smax_l"],total_samples=total_samples,std_factor=std_factor,size_of_crop=size_of_crop)
                for index,roi in enumerate(roi_objects):
                    new_image = nibabel.Nifti1Image(roi.astype(np.float64), affine=np.eye(4))
                    nibabel.save(new_image, save_path_smax_l + f"/{index}_" + "smax_l.nii.gz")

                roi_objects = __get__crop__(img.get_fdata(),cfg["smax_r"],total_samples=total_samples,std_factor=std_factor,size_of_crop=size_of_crop,flip=True)
                for index,roi in enumerate(roi_objects):
                    new_image = nibabel.Nifti1Image(roi.astype(np.float64), affine=np.eye(4))
            ...
            for index, row in tqdm(df.iterrows(),total=len(df)):
                ...
                if row["Kieferhöhle"] == "NA":
                    ...
                    try:
                        ...
                        save_path_l = root_save_path+f"/{patient_id}/normal/"
                        save_path_r = root_save_path+f"/{patient_id}/normal/"
                    ...
                    for cat in categories:
                        ...
                        if "l" in cat:
                            ...
                            if "1" in cat: 
                                ...
                                save_path = root_save_path+f"/{patient_id}/mucosal_thickening/"
                            ...
                            if "2" in cat: 
                                save_path = root_save_path+f"/{patient_id}/polyps/"
                                create_dir(save_path)
                                patient_list_bad.append(patient_id)
                            ...
                            if "3" in cat: 
                                save_path = root_save_path+f"/{patient_id}/cysts/"
                                create_dir(save_path)
                            ...
                            if "5" in cat: 
                                save_path = root_save_path+f"/{patient_id}/fully_occupied/"
                                #create_dir(save_path)
                                #patient_list_bad.append(patient_id)
                    ...
                    if not right_bad:
                        ...
                        save_path = root_save_path+f"/{patient_id}/normal/"
                        create_dir(save_path)
                        #extracting right maxillary sinus volumes
                        roi_objects = __get__crop__(img.get_fdata(),cfg["smax_r"],total_samples=total_samples,std_factor=std_factor,size_of_crop=size_of_crop,flip=True)
                        occurrance["no_path"]["smax_r"] +=  1
