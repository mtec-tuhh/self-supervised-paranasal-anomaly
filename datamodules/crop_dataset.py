...
def __get__crop__(data,location,flip=False):
    ...
    smin_start = smin-size_around
    
    out = data[smin-size_around:smax+size_around,cmin-size_around:cmax+size_around,amin-size_around:amax+size_around]
...
roi_dataset =  "/media/debayan/c7b64c90-ca4e-4192-8ed9-8fea1d005196/MRI_HCHS/dataset/roi_dataset_SPIE_large_fov/{}/{}.nii.gz"
...
for index, row in df.iterrows():
    ...
    if row["smax"] == "NA":
        ...
        nibabel.save(new_image,roi_dataset.format("good",patient_id + "_0_smax_l"))

        roi = __get__crop__(img.get_data(),cfg["smax_r"],flip=True)
        new_image = nibabel.Nifti1Image(roi.astype(np.float), affine=np.eye(4))
        nibabel.save(new_image,roi_dataset.format("good",patient_id + "_0_smax_r"))
        ...
        for cat in categories:
            ...
            if "l" in cat:
                ...
                if "1" in cat: 
                    ...
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_1_smax_l"))
                    
                if "2" in cat: 
                    occurrance["2"]["smax_l"] +=  1
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_2_smax_l"))
                    
                if "3" in cat: 
                    occurrance["3"]["smax_l"] +=  1
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_3_smax_l"))
                ...
                if "1" in cat: 
                    ...
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_1_smax_r"))
                    
                if "2" in cat: 
                    occurrance["2"]["smax_r"] +=  1
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_2_smax_r"))
                    
                if "3" in cat: 
                    occurrance["3"]["smax_r"] +=  1
                    nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_3_smax_r"))
                ...
                left_bad = True
                roi = __get__crop__(img.get_data(),cfg["smax_l"])
                new_image_l = nibabel.Nifti1Image(roi.astype(np.float), affine=np.eye(4))
                #nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_"+str(uuid.uuid1())))

                right_bad  = True 
                roi = __get__crop__(img.get_data(),cfg["smax_r"],flip=True)
                new_image_r = nibabel.Nifti1Image(roi.astype(np.float), affine=np.eye(4))
                #nibabel.save(new_image,roi_dataset.format("bad",patient_id + "_"+str(uuid.uuid1())))
                if "1" in cat: 
                    occurrance["1"]["smax_l"] +=  1
                    nibabel.save(new_image_l,roi_dataset.format("bad",patient_id + "_1_smax_l"))
                if "2" in cat: 
                    occurrance["2"]["smax_l"] +=  1
                    nibabel.save(new_image_l,roi_dataset.format("bad",patient_id + "_2_smax_l"))
                if "3" in cat: 
                    occurrance["3"]["smax_l"] +=  1
                    nibabel.save(new_image_l,roi_dataset.format("bad",patient_id + "_3_smax_l"))

                if "1" in cat: 
                    occurrance["1"]["smax_r"] +=  1
                    nibabel.save(new_image_r,roi_dataset.format("bad",patient_id + "_1_smax_r"))
                if "2" in cat: 
                    occurrance["2"]["smax_r"] +=  1
                    nibabel.save(new_image_r,roi_dataset.format("bad",patient_id + "_2_smax_r"))
                if "3" in cat: 
                    occurrance["3"]["smax_r"] +=  1
                    nibabel.save(new_image_r,roi_dataset.format("bad",patient_id + "_3_smax_r"))
